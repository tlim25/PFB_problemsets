...
# definindo as traduções dos códons
	...
	longest = max(translation, key=len)+'M'
	for i in range (0, len(longest), 3):
		subset=[]
		subset.append(longest[i:i+3])
	print(gene,'-frame1-subset-longest peptide\n',subset)
	for codon in complementar:
		if len(codon) == 3:
			translationComp.append(translation_table[codon])
	translationComp = ''.join(translationComp).split('M')
	longestComp = max(translationComp, key=len)+'M'
	for i in range (0, len(longestComp), 3):
		subsetComp = []
		subsetComp.append(longestComp[i:i+3])
	print(gene,'-frame1-compsubset-longest peptide\n',longestComp)
